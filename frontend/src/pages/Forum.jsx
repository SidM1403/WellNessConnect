      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            ...
            >
              ...
              <span className="text-sm font-bold tracking-wide text-primary-600 uppercase">Community Forum</span>
            </motion.div>
            <motion.h1
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-5 rounded-3xl shadow-sm border border-surface-200 sticky top-32">
              ...
              <div className="space-y-4">
                <div className="relative">
                  <FaSearch className="absolute left-3 top-3 text-text-light" />
                  <input
                    placeholder="Search topics..."
                    className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-surface-50 border border-surface-200 text-sm focus:ring-2 focus:ring-primary-200 focus:border-primary-400 outline-none transition-all"
                ...
                <div>
                  <label className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2 block">Tags</label>
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => setFilters({ ...filters, tag: '' })}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filters.tag === '' ? 'bg-primary-500 text-white shadow-md' : 'bg-surface-100 text-text-secondary hover:bg-surface-200'
                        }`}
                    >
                      All
                    </button>
                    {tags.map(t => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filters.tag === t ? 'bg-primary-500 text-white shadow-md' : 'bg-surface-100 text-text-secondary hover:bg-surface-200'
                          }`}
                      >
                        {t}
                      </button>
          ...
          <div className="lg:col-span-3 space-y-6">
            <AnimatePresence>
              {showCreate && (
                <motion.form
                  ...
                  className="bg-white p-6 rounded-[2rem] shadow-lg border border-primary-100 overflow-hidden"
                  onSubmit={create}
                >
                  <h3 className="font-bold text-lg text-text-primary mb-4">Create a new discussion</h3>
                  <div className="space-y-4">
                    <input
                      className="w-full px-4 py-3 bg-surface-50 rounded-xl border border-surface-200 focus:ring-2 focus:ring-primary-200 outline-none transition-all"
                    ...
                    />
                    <textarea
                      className="w-full px-4 py-3 bg-surface-50 rounded-xl border border-surface-200 focus:ring-2 focus:ring-primary-200 outline-none transition-all min-h-[100px]"
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {tags.map(t => (
                        <label key={t} className={`cursor-pointer px-3 py-1 rounded-full text-xs font-medium border transition-all ${form.tags.includes(t)
                            ...
                            : 'bg-white border-surface-200 text-text-secondary hover:border-primary-200'
                          }`}>
                          <input
                            type="checkbox"
                            className="hidden"
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <div className="flex gap-3">
                        <button type="button" onClick={() => setShowCreate(false)} className="px-5 py-2 text-text-secondary font-medium hover:bg-surface-100 rounded-xl transition-colors">Cancel</button>
            ...
            <div className="space-y-6">
              {loading ? (
                <div className="text-center py-12 text-text-light">Loading community...</div>
              ) : (
                <>
                  {posts.map((p) => (
                    <PostCard key={p._id} post={p} onLike={like} />
                  ))}
                  {posts.length === 0 && <div className="text-center py-12 text-text-secondary italic bg-white rounded-3xl border border-surface-100">No discussions found. Be the first to start one!</div>}
                </>
              )}
            </div>
          </div>
